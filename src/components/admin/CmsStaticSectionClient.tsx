...
export function CmsStaticSectionClient({
  section,
  initialPages,
}: {
  section: 'home' | 'about' | 'contacts' | 'footer'
  initialPages: PagesContent
}) {
  ...
  const editor =
    section === 'home' ? (
      <HomePagesEditor pages={pages} setPages={setPages} L={L} />
    ) : section === 'about' ? (
      <AboutPagesEditor pages={pages} setPages={setPages} L={L} />
    ) : section === 'contacts' ? (
      <ContactsPagesEditor pages={pages} setPages={setPages} L={L} />
    ) : (
      <FooterTemplateEditor pages={pages} setPages={setPages} L={L} />
    )
    ...
    <main className="min-h-0 flex-1 overflow-y-auto">
      <div className="mx-auto max-w-4xl px-6 py-8 md:px-10 md:py-10">
        ...
        <div className="mb-8 flex flex-col gap-4 border-b border-neutral-800 pb-6 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-white">{SECTION_LABEL[section]}</h1>
            <p className="mt-1 text-sm text-neutral-500">Static page content (Redis JSON).</p>
          </div>
