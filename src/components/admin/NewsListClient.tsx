...
import { createNewsDraftAction } from '@/lib/actions/admin-actions'
  ...
  const filtered = useMemo(() => {
    ...
    return initialNews
      ...
      .filter((n) => {
        ...
        const t =
          `${pickLang(n.title, 'en')} ${pickLang(n.title, 'uk')} ${n.slug} ${n.id}`.toLowerCase()
  ...
  return (
    <main className="min-h-0 flex-1 overflow-y-auto">
      <div className="mx-auto max-w-5xl px-6 py-8 md:px-10 md:py-10">
        <AdminBreadcrumbs
          items={[{ label: 'News', href: '/admin/news' }, { label: 'All posts' }]}
        />
        <div className="mb-8 flex flex-col gap-4 border-b border-neutral-800 pb-6 md:flex-row md:items-end md:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-white">All news</h1>
            <p className="mt-1 text-sm text-neutral-500">Bilingual posts with Markdown body.</p>
          </div>
            ...
            <li key={n.id}>
              ...
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-neutral-100">{pickLang(n.title, 'en')}</p>
                  <p className="truncate text-xs text-neutral-500">
                    /news/{n.slug} · {n.publishedAt}
                  </p>
                </div>
                {statusPill(n.status)}
              </Link>
