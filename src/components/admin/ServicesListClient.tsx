...
import { createServiceDraftAction } from '@/lib/actions/admin-actions'
  ...
  const filtered = useMemo(() => {
    ...
    return initialServices
      ...
      .filter((s) => {
        ...
        const t = `${pickLang(s.title, 'en')} ${pickLang(s.title, 'uk')} ${s.href} ${s.id}`.toLowerCase()
  ...
  return (
    <main className="min-h-0 flex-1 overflow-y-auto">
      <div className="mx-auto max-w-5xl px-6 py-8 md:px-10 md:py-10">
        <AdminBreadcrumbs
          items={[{ label: 'Services', href: '/admin/services' }, { label: 'All services' }]}
        />
        <div className="mb-8 flex flex-col gap-4 border-b border-neutral-800 pb-6 md:flex-row md:items-end md:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight text-white">All services</h1>
            <p className="mt-1 text-sm text-neutral-500">Each service opens in its own editor.</p>
          </div>
        ...
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Search title, href, id…"
            ...
            <li key={s.id}>
              ...
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-neutral-100">{pickLang(s.title, 'en')}</p>
                  <p className="truncate text-xs text-neutral-500">{s.href}</p>
                </div>
                {statusPill(s.status)}
              </Link>
            </li>
